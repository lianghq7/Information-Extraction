...
def build_original_table(current_td_div): #根据当前找到的表格的第一项，搜索出整个表格并输出
    ...
    while current_td_div:
        ...
        if (refinish_tag == 1 and len(re.findall(r'[\u4e00-\u9fa5]+',current_td_div.get_text())) > 0) or \
                (begin_tag == 1 and len(re.findall(r'\d\d.{1,4}[\u4e00-\u9fa5]{4,}', current_td_div.get_text())) > 0 and len(re.findall(r'[年发生额度]', current_td_div.get_text())) == 0) or (begin_tag == 1 and len(re.findall(r'(\b管\s*理\s*费\s*用\b)|(财\s*务\s*报\s*表\s*附\s*注)|(\b财\s*务\s*费\s*用\b)|(\b说\s*明\s*：?)|(\b注\s*：?)', current_td_div.get_text())) == 1)or\
                ((len(re.findall(r'销售费用', current_td_div.get_text())) > 0) and (len(re.findall(r'(\b销\s*售\s*费\s*用\b)|(\b销\s*售\s*费\s*用\s*合\s*计\b)|(\b其\s*他\s*销\s*售\s*费\s*用\b)|(\b技\s*术\s*支\s*持\s*销\s*售\s*费\s*用\b)|(\b黄\s*金\s*销\s*售\s*费\s*用\b)|(\b产\s*品\s*销\s*售\s*费\s*用\b)|(\b商\s*城\s*销\s*售\s*费\s*用\b)|(\b销\s*售\s*费\s*用\s*明\s*细\s*合\s*计\b)', current_td_div.get_text())) == 0)):
...
def information_extract(html_filename):
    ...
    def else_inEX_else(): #未读到表格的处理
                    ...
                    if countlen > 0:
                        ...
                        if len(re.findall(r'[^\s]', div_temp_str)) == 0 and ((len(
                            ...
                            if (len(re.findall(r'[\u4e00-\u9fa5]', div_temp_str2)) == 4) and ((len(
                                ...
                                if ((len(re.findall(r'[\u4e00-\u9fa5]', div_temp_str2)) > 4) and (len(re.findall(r'\d|[一二三四五六七八九十]', div_temp_str2)) > 0)) or ((len(re.findall(r'[\u4e00-\u9fa5]', div_temp_str2)) == 4) and (len(re.findall(r'\d', div_temp_str2)) > 0)):
                                    current_div2 = div_temp;
                                    is_end = 1
                                    break

        max_search_range2 = 5  # 最大查找范围，如果超过这个数仍然找不到结束标志，则可以结束查找
        ...
        #             print(div_temp.get_text())
        #             #匹配条件找入口
        #             if len(re.findall(r'[^\s]', div_temp_str)) == 0 and ((len(re.findall(r'[\u4e00-\u9fa5]', div_temp_str2))-len(re.findall(r'[一二三四五六七八九十]', div_temp_str2))) < 8) and (len(re.findall(r'\d', previous_div_str)) > 3 or len(re.findall(r'\d', previous_div_str)) == 0):
        #                 if(len(re.findall(r'[\u4e00-\u9fa5]', div_temp_str2)) == 4) and ((len(re.findall(r'[\u4e00-\u9fa5]', find_next_div(div_temp).get_text()))-len(re.findall(r'[一二三四五六七八九十]', find_next_div(div_temp).get_text()))) <3) and (len(re.findall(r'\d', div_temp_str2)) == 0) :
        #                     current_div = div_temp;
        #                     is_end = 1
        #                     break
        #                 else:
        ...
        for i in range(len(pf) - 2, int(len(pf) * 0.5), -1):
            ...
            for div_temp in page_temp:
                ...
                if countlen > 0:
                    ...
                    if ((len(re.findall(r'\d|[一二三四五六七八九]', div_temp_str)) > 0) and (len(re.findall(r'\d|[一二三四五六七八九]', div_temp_str)) < 3) and (len(re.findall(r'[\u4e00-\u9fa5]', div_temp_str)) < 4) and (len(re.findall(r'[\u4e00-\u9fa5]', div_temp_str2)) < 10) and (len(re.findall(r'\d', div_temp_str2)) < 3)) or\
                            ((len(re.findall(r'\d|[一二三四五六七八九]', div_temp_str)) == 0)and (len(re.findall(r'\d|[一二三四五六七八九]', previous_div_str)) > 0) and (len(re.findall(r'\d|[一二三四五六七八九]', previous_div_str)) < 3) \
                                     and (len(re.findall(r'[\u4e00-\u9fa5]', div_temp_str)) < 4)and (len(re.findall(r'[\u4e00-\u9fa5]', previous_div_str)) < 4) and (len(re.findall(r'[\u4e00-\u9fa5]', div_temp_str2)) < 10) and (len(re.findall(r'\d', div_temp_str2)) < 3)):
                            if len(re.findall(r'\d', div_temp_str)) == 1:
                                if ((re.findall(r'\d', div_temp_str))[0] != '1') and (
                                    (re.findall(r'\d', div_temp_str))[0] != '2'):
                                    current_div = div_temp
                                    break
                            else:
                                current_div = div_temp
                ...
                for div_temp in selling_expenses_page_content:
                    ...
                    if countlen > 0:
                        ...
                        if (len(re.findall(r'\d|[一二三四五六七八九]', div_temp_str)) > 0 and (len(re.findall(r'\d|[一二三四五六七八九]' \
                                , div_temp_str)) < 3)and (len(re.findall(r'[\u4e00-\u9fa5]', div_temp_str)) < 4) and (len(re.findall(r'[\u4e00-\u9fa5]', div_temp_str2)) < 10)and (len(re.findall(r'\d', div_temp_str2)) < 3)) or ((len(re.findall(r'\d|[一二三四五六七八九]', div_temp_str)) == 0) and \
                                                                                                                                                         (len(re.findall(r'\d|[一二三四五六七八九]',
                                                                                     previous_div_str)) > 0 ) and (len(
                            re.findall(r'\d|[一二三四五六七八九]', previous_div_str)) < 3 )and (len(re.findall(r'[\u4e00-\u9fa5]', div_temp_str)) < 4)and (len(re.findall(r'[\u4e00-\u9fa5]', previous_div_str)) < 4) and (len(re.findall(r'[\u4e00-\u9fa5]', div_temp_str2)) < 10)and (len(re.findall(r'\d', div_temp_str2)) < 3)):
                            if len(re.findall(r'\d', div_temp_str)) == 1:
                                if ((re.findall(r'\d', div_temp_str))[0] != '1') and (
                                    (re.findall(r'\d', div_temp_str))[0] != '2'):
                                    current_div = div_temp
    ...
    max_search_range = 6  # 最大查找范围，如果超过这个数仍然找不到表格，则可以认为待查表格不在附近
    ...
    while current_div and max_search_range != 0:
        ...
        if (current_div != title_div and (len(re.findall(r'\d\d.{1,4}[\u4e00-\u9fa5]{4,}', current_div.get_text())) == 1) and (len(re.findall(r'[年发生额度]',current_div.get_text())) == 0)) or len(re.findall(r'\b管理费用\b',current_div.get_text())) > 0 :
            break
        if len(re.findall(r'\b项\s*目\b',current_div.get_text())) > 0 and len(re.findall(r'(全\s*文)|(报\s*告)',current_div.get_text())) == 0 :
    ...
    if not first_td_div:
        ...
        #                 previous_div_str = find_previous_div(div_temp).get_text()
        #             #匹配条件找入口
        #             if len(re.findall(r'[^\s]', div_temp_str)) == 0 and ((len(re.findall(r'[\u4e00-\u9fa5]', div_temp_str2))-len(re.findall(r'[一二三四五六七八九十]', div_temp_str2))) < 8) and len(re.findall(r'\d', previous_div_str)) > 3:
        #                 if(len(re.findall(r'[\u4e00-\u9fa5]', div_temp_str2)) == 4) and ((len(re.findall(r'[\u4e00-\u9fa5]', previous_div_str))-len(re.findall(r'[一二三四五六七八九十]', previous_div_str))) <3) :
        #                     current_div = div_temp;
        #                     is_end = 1
        #                     break
        #                 else:
        #                     if len(re.findall(r'[\u4e00-\u9fa5]', div_temp_str2)) > 4:
        #                         current_div = div_temp;
        #                         is_end = 1
        #                         break
        #
        # table2str = None
        # max_search_range = 20  # 最大查找范围，如果超过这个数仍然找不到结束标志，则可以结束查找
        # while (current_div!= None) and (len(re.findall(r'\b管理费用\b',current_div.get_text())) == 0 )and (len(re.findall(r'\b财务费用\b',current_div.get_text())) == 0) and (max_search_range != 0):
        #     table2str += str(current_div)
        #     current_div = find_next_div(current_div)
        #     max_search_range -= 1
        ...
        # if else_inEX_else() != '':
        #     table2str_temp1P = re.compile(r'<[^>]*span[^>]*>', re.S)
        #     table2str_temp1 = table2str_temp1P.sub('', else_inEX_else())
        #     table2str_tempP = re.compile(r'<[^>]+>', re.S)
        #     table2str = table2str_tempP.sub(' ', table2str_temp1)
...
path = ["html/2007/YEAR/","html/2008/YEAR/","html/2009/YEAR/"]
fiscal_period_dict = {"html/2016/YEAR/": "2016-12-31", "html/2015/YEAR/": "2015-12-31", "html/2007/YEAR/": "2007-12-31", \
                      "html/2008/YEAR/": "2008-12-31", "html/2009/YEAR/": "2009-12-31"}
